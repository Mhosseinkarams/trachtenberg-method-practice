...
class FastMathApp:
    def __init__(self, page: ft.Page):
        ...
        # Performance Optimization: Cache generated views for faster navigation
        self.cached_views = {}
    ...
    def toggle_language(self, e):
        ...
        self.update_page_config()
        # Performance Optimization: Clear view cache on language toggle
        self.cached_views = {}
        # Full UI refresh
    ...
    def show_system_selection(self, update: bool = True):
        ...
        # Performance Optimization: Return cached view if available
        if "system_selection" in self.cached_views:
            self.main_content.controls.append(self.cached_views["system_selection"])
        ...
        self.cached_views["system_selection"] = view
        self.main_content.controls.append(view)
        if update:
            self.page.update()
    ...
    def show_categories(self, update: bool = True):
        ...
        for cat_key, info in CATEGORIES_INFO.items():
            # Performance Optimization: Use pre-calculated rules_by_system_category
    ...
    def show_rule_selector(self, category: str, update: bool = True):
        ...
        # Performance Optimization: Return cached view if available
        cache_key = f"rules_{self.selected_system}_{category}"
        if cache_key in self.cached_views:
            self.main_content.controls.append(self.cached_views[cache_key])
            if update: self.page.update()
            return
        ...
        view = ft.Column([back_button, grid])
        self.cached_views[cache_key] = view
        self.main_content.controls.append(view)
        if update:
            self.page.update()
    ...
    def show_mode_selection(self, update: bool = True):
        ...
        # Performance Optimization: Return cached view if available
        cache_key = f"mode_{self.selected_rule.id}"
        if cache_key in self.cached_views:
            self.main_content.controls.append(self.cached_views[cache_key])
            if update: self.page.update()
            return
        ...
        view = ft.Column([
            back_button,
            ft.Text(f"{ui['target']} {self.selected_rule.get_name(self.lang)}", size=20, weight=ft.FontWeight.BOLD, color=COLOR_PRIMARY),
            cards
        ])
        self.cached_views[cache_key] = view
        self.main_content.controls.append(view)
    ...
    async def next_problem(self, e=None):
        ...
        # Performance Optimization: Granular updates for practice area
    ...
    async def check_answer(self, e):
        ...
        # Performance Optimization: Granular update instead of page.update()
        self.practice_card.update()
