...
class FastMathApp:
    def __init__(self, page: ft.Page):
        ...
        # Performance Optimization: Cache for UI views to avoid redundant reconstruction
        self.cached_views = {}
    ...
    def show_system_selection(self, update: bool = True):
        ...
        if "systems" in self.cached_views:
            self.main_content.controls.append(self.cached_views["systems"])
        ...
        self.cached_views["systems"] = view
    ...
    def show_categories(self, update: bool = True):
        ...
        for cat_key, info in CATEGORIES_INFO.items():
            # Performance Optimization: Use pre-calculated index
    ...
    def show_mode_selection(self, update: bool = True):
        self.main_content.controls.clear()
        cache_key = f"modes_{self.selected_rule.id}"
        if cache_key in self.cached_views:
            self.main_content.controls.append(self.cached_views[cache_key])
            if update: self.page.update()
            return
    ...
    async def next_problem(self, e=None):
        ...
        # Performance Optimization: Use granular update
    ...
    async def check_answer(self, e):
        ...
        # Performance Optimization: Use granular update to minimize socket traffic
        self.practice_card.update()
