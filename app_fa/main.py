...
class FastMathApp:
    ...
    def setup_ui(self, update: bool = True):
        ...
        self.page.add(
            ft.Column([
                ...
                self.main_content,
                ft.Divider(),
                ft.Text("© 2024 مربی محاسبات سریع. Built with Flet.", size=12, color=ft.Colors.GREY_400, text_align=ft.TextAlign.CENTER)
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER)
        )
        if update:
            self.page.update()
        ...
        while self.timer_running and self.timer_id == tid:
            if self.start_time and hasattr(self, "timer_text"):
                try:
                    ...
                    self.timer_text.value = f"زمان: {mins:02d}:{secs:02d}"
    ...
    def show_practice_area(self, update: bool = True):
        ...
        self.score_text = ft.Text(f"امتیاز: 0/0", size=16)
        self.streak_text = ft.Text(f"توالی: 0", size=16, color=ft.Colors.ORANGE_800, weight=ft.FontWeight.BOLD)
        self.timer_text = ft.Text(f"زمان: 00:00", size=16)
    ...
    async def check_answer(self, e):
        ...
        if user_val == self.current_problem["answer"]:
            ...
            self.streak = 0
            self.feedback_text.value = f"اشتباه بود. جواب {to_persian_digits(self.current_problem['answer'])} بود."
            self.feedback_text.color = ft.Colors.RED_600
            self.practice_card.bgcolor = ft.Colors.RED_50

        self.score_text.value = f"امتیاز: {self.score}/{self.total}"
        self.streak_text.value = f"توالی: {self.streak}"
